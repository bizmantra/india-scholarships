...
def check_url(url):
    try:
        # Use GET or HEAD. Some servers block HEAD, so let's try HEAD first, then GET if it fails or returns 405.
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
        }
        # First send request without redirect follow to see initial response
        ...
        # Follow redirects to get final destination and status
...
def main():
    wb = openpyxl.load_workbook('tmp/gsc_discovered.xlsx')
    ...
    with open("tmp/gsc_url_check_report.txt", "w") as f:
        f.write(f"GSC Discovered URLs Check Report\n")
