# 📊 Content Quality Audit Report
Generated on: 2026-09-13
Total Scholarships Audited: **482**
Scholarships with Issues: **116** (24.1%)

---

## 📈 Executive Summary

Below is a breakdown of the content issues discovered across all scholarship pages:

| Metric / Content Area | Number of Affected Scholarships | % of Total | Description |
| :--- | :---: | :---: | :--- |
| **Legacy Flagged** | 0 | 0.0% | Marked with `[LEGACY]` in title or slug |
| **Missing Annual Amount** | 0 | 0.0% | Missing/0 annual amount (causes "upto 0k" display) |
| **Missing Min Amount** | 62 | 12.9% | Missing/0 minimum amount |
| **Missing Deadline Date** | 1 | 0.2% | Deadline is empty or "Not specified" |
| **Expired Deadline** | 0 | 0.0% | Deadline is in the past (before 2026-06-26) |
| **Old Year References** | 25 | 5.2% | Mentions 2024, 2025, or earlier cycles |
| **Incomplete Selection Criteria** | 0 | 0.0% | Missing or under 15 characters |
| **Incomplete Renewal Policy** | 0 | 0.0% | Missing or under 15 characters |
| **Incomplete Step Guide** | 0 | 0.0% | Missing or under 20 characters |
| **Missing Documents** | 15 | 3.1% | No required documents listed |
| **Missing / Bad Apply Link** | 0 | 0.0% | No official website or application URLs |
| **Missing Helpline** | 24 | 5.0% | Helpline is empty, "Not Specified", or generic |
| **Missing FAQs** | 0 | 0.0% | FAQ block is empty or missing |
| **Contains Raw HTML** | 1 | 0.2% | HTML tags (like `<p>`, `<a>`) in text fields |

---

## 🔍 Detail of Top Affected Scholarships (Sorted by Issue Count)

Here are the scholarships with the highest number of content quality issues:

| Slug | Title | Issues Count | Key Gaps |
| :--- | :--- | :---: | :--- |
| `mahadbt-post-matric-scholarship-maharashtra` | **MahaDBT Post-Matric Scholarship (Maharashtra)** | 3 | Old Year Reference (e.g. 2024 or 2025 in title or description); Missing Required Documents; Missing Helpline Contact Details |
| `holland-scholarship` | **Holland Scholarship** | 3 | Missing Min Amount; Old Year Reference (e.g. 2024 or 2025 in title or description); Invalid Apply URL format: "Applications are made directly through the website of the chosen participating Dutch higher education institution. There is no single central application portal for the NL Scholarship. Always check the institution's official website for specific application instructions and deadlines." |
| `nec-merit-scholarship` | **NEC Merit Scholarship** | 2 | Missing Required Documents; Missing Helpline Contact Details |
| `kotak-shiksha-nidhi-scholarship` | **Kotak Shiksha Nidhi Scholarship** | 2 | Missing Required Documents; Missing Helpline Contact Details |
| `sanchi-honnamma-scholarship-karnataka` | **Sanchi Honnamma Scholarship (Karnataka)** | 2 | Old Year Reference (e.g. 2024 or 2025 in title or description); Missing Required Documents |
| `lila-poonawalla-foundation-scholarship` | **Lila Poonawalla Foundation Scholarship** | 2 | Missing Required Documents; Missing Helpline Contact Details |
| `foundation-for-excellence-ffe-scholarship` | **Foundation for Excellence (FFE) Scholarship** | 2 | Old Year Reference (e.g. 2024 or 2025 in title or description); Missing Required Documents |
| `tata-trusts-medical-and-healthcare-scholarship` | **Tata Trusts Medical and Healthcare Scholarship 2026-27** | 2 | Old Year Reference (e.g. 2024 or 2025 in title or description); Contains Unwanted Raw HTML Tags |
| `central-sector-scholarship-kerala-applicants-via-dce` | **Central Sector Scholarship (Kerala applicants via DCE)** | 2 | Old Year Reference (e.g. 2024 or 2025 in title or description); Missing Helpline Contact Details |
| `post-matric-scholarship-for-differently-abled` | **Post-Matric Scholarship for Differently Abled** | 2 | Missing Deadline Date; Old Year Reference (e.g. 2024 or 2025 in title or description) |
| `pragyan-bharati-scheme` | **Pragyan Bharati Scheme** | 2 | Missing Min Amount; Missing Helpline Contact Details |
| `uttarakhand-state-merit-scholarship` | **Uttarakhand State Merit Scholarship** | 2 | Old Year Reference (e.g. 2024 or 2025 in title or description); Missing Helpline Contact Details |
| `manipur-state-merit-scholarship` | **Manipur State Merit Scholarship** | 2 | Old Year Reference (e.g. 2024 or 2025 in title or description); Missing Helpline Contact Details |
| `tripura-state-merit-scholarship` | **Tripura State Merit Scholarship** | 2 | Old Year Reference (e.g. 2024 or 2025 in title or description); Missing Helpline Contact Details |
| `mahindra-all-india-talent-scholarship` | **Mahindra All India Talent Scholarship** | 2 | Old Year Reference (e.g. 2024 or 2025 in title or description); Missing Helpline Contact Details |
| `teach-for-india-fellowship` | **Teach For India Fellowship** | 2 | Old Year Reference (e.g. 2024 or 2025 in title or description); Missing Helpline Contact Details |
| `aga-khan-foundation-international-scholarship-programme` | **Aga Khan Foundation International Scholarship Programme** | 2 | Missing Min Amount; Invalid Apply URL format: "Applications are decentralized and forms are obtained from local Aga Khan Foundation (AKF) or Aga Khan Education Services/Boards offices in the applicant's country of current residence. There is no central online application portal. The 2026-2027 application cycle for studies commencing in Fall 2026 is closed. The next cycle will open in early 2027 for studies commencing in Fall 2027." |
| `sheffield-academy-scholarships` | **Sheffield Academy Scholarships** | 2 | Missing Min Amount; Missing Required Documents |
| `commonwealth-masters-scholarships` | **Commonwealth Masters Scholarships** | 2 | Missing Min Amount; Invalid Apply URL format: "The official application portal is CSC Central, accessible via the Commonwealth Master's Scholarships page on the CSC website when applications are open. For 2026/2027, the application deadline was generally October 14 or 21, 2025. Please refer to your nominating agency for specific opening and closing dates." |
| `green-passage-scheme-technical-education-odisha` | **Green Passage Scheme - Technical Education (Odisha)** | 1 | Missing Min Amount |
| `videsh-siksha-bruti-foreign-education-scholarship-odisha` | **Videsh Siksha Bruti - Foreign Education Scholarship (Odisha)** | 1 | Missing Min Amount |
| `kc-mahindra-scholarship-for-post-graduate-studies-abroad` | **KC Mahindra Scholarship for Post-Graduate Studies Abroad** | 1 | Old Year Reference (e.g. 2024 or 2025 in title or description) |
| `mukhyamantri-medhavi-vidyarthi-yojana-mmvy` | **Mukhyamantri Medhavi Vidyarthi Yojana (MMVY)** | 1 | Missing Min Amount |
| `dr-ambedkar-medhavi-chattervriti-yojana` | **Dr. Ambedkar Medhavi Chattervriti Yojana** | 1 | Missing Min Amount |
| `post-matric-merit-scholarship-pmms-mizoram` | **Post-Matric Merit Scholarship (PMMS) Mizoram** | 1 | Missing Helpline Contact Details |
| `sant-sohirobanath-ambiye-bursary-scheme` | **Sant Sohirobanath Ambiye Bursary Scheme** | 1 | Missing Min Amount |
| `perunthalaivar-kamarajar-financial-assistance-scheme-pkfas` | **Perunthalaivar Kamarajar Financial Assistance Scheme (PKFAS)** | 1 | Missing Helpline Contact Details |
| `additional-scholarship-for-st-students-andaman-nicobar` | **Additional Scholarship for ST Students (Andaman & Nicobar)** | 1 | Missing Helpline Contact Details |
| `lakshadweep-scholarship-scheme` | **Lakshadweep Scholarship Scheme** | 1 | Missing Helpline Contact Details |
| `pm-yasasvi-post-matric-scholarship-dnh-dd` | **PM Yasasvi Post-Matric Scholarship (DNH & DD)** | 1 | Missing Helpline Contact Details |
| `national-overseas-scholarship-for-scst-students` | **National Overseas Scholarship for SC/ST Students** | 1 | Missing Helpline Contact Details |
| `inspire-scholarship-for-higher-education-she` | **INSPIRE Scholarship for Higher Education (SHE)** | 1 | Missing Helpline Contact Details |
| `aicte-saksham-scholarship-for-students-with-disabilities` | **AICTE Saksham Scholarship for Students with Disabilities** | 1 | Missing Helpline Contact Details |
| `aicte-swanath-scholarship-scheme` | **AICTE Swanath Scholarship Scheme** | 1 | Missing Helpline Contact Details |
| `national-fellowship-scholarship-for-higher-education-of-st-students` | **National Fellowship & Scholarship for Higher Education of ST Students** | 1 | Missing Required Documents |
| `pre-matric-scholarship-for-students-with-disabilities` | **Pre-Matric Scholarship for Students with Disabilities** | 1 | Missing Required Documents |
| `pm-scholarship-scheme-railways` | **PM Scholarship Scheme (Railways)** | 1 | Missing Required Documents |
| `tata-trusts-education-grants` | **Tata Trusts Education Grants** | 1 | Missing Required Documents |
| `vidyasaarathi-scholarship-csr-programs` | **Vidyasaarathi Scholarship (CSR Programs)** | 1 | Missing Required Documents |
| `ssp-pre-matric-post-matric-scholarship-karnataka` | **SSP Pre-Matric & Post-Matric Scholarship (Karnataka)** | 1 | Missing Required Documents |
| `nmms-scholarship-karnataka` | **NMMS Scholarship (Karnataka)** | 1 | Missing Required Documents |
| `nmms-scholarship-maharashtra` | **NMMS Scholarship (Maharashtra)** | 1 | Missing Required Documents |
| `sbi-platinum-jubilee-asha-scholarship` | **SBI Platinum Jubilee Asha Scholarship 2026-27** | 1 | Old Year Reference (e.g. 2024 or 2025 in title or description) |
| `narotam-sekhsaria-postgraduate-scholarship` | **Narotam Sekhsaria Postgraduate Scholarship 2026** | 1 | Old Year Reference (e.g. 2024 or 2025 in title or description) |
| `glow-lovely-careers-scholarship-for-women` | **Glow & Lovely Careers Scholarship for Women 2026-27** | 1 | Old Year Reference (e.g. 2024 or 2025 in title or description) |
| `junior-merit-scholarship` | **Junior Merit Scholarship** | 1 | Old Year Reference (e.g. 2024 or 2025 in title or description) |
| `goa-dayanand-social-security-scheme-students` | **Goa Dayanand Social Security Scheme (Students)** | 1 | Missing Helpline Contact Details |
| `meghalaya-merit-scholarship` | **Meghalaya Merit Scholarship** | 1 | Missing Helpline Contact Details |
| `arunachal-pradesh-state-merit-scholarship` | **Arunachal Pradesh State Merit Scholarship** | 1 | Missing Helpline Contact Details |
| `ladakh-ut-merit-scholarship` | **Ladakh UT Merit Scholarship** | 1 | Missing Helpline Contact Details |

*Note: Showing top 50 rows. A complete list of all 116 records is exported to [content-quality-audit.csv](file:///home/runner/work/india-scholarships/india-scholarships/scholarship-app/data/content-quality-audit.csv).*
