...
*   **Database**:
    ...
    *   **Local working copy**: `data/scholarships.db` is NOT in git. Download it with `npm run db:pull`; send changes with `npm run db:push`. Builds pull it from Turso automatically before `next build`. (Historical note: it used to be committed.)
    *   **(Historical) Local / Build Fallback**: A local SQLite file (`data/scholarships.db`) used to be checked directly into the git repository. When the production database has query or latency issues, or during offline local rendering, it falls back to this file (`file:data/scholarships.db`).
